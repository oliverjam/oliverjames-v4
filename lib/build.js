...
/**
 * [2] Read pages/ dir to find all templates
 */
let files = walk(options.pages_dir, { includeDirs: false, exts: [".jsx"] });

/**
 * [3] Run all templates to write all pages
 */
await map_async(files, write_page);

/**
 * [4] Move favicon to root of output folder
 */
let favicon = await Deno.readFile("favicon.ico");
await write_binary({
  path: join(options.out_dir, "favicon.ico"),
  content: favicon,
});
